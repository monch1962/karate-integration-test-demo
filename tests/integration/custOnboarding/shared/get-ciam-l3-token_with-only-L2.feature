Feature:
  ...
  Scenario: Get Level 3 token
	...
	* def lvl2TokenResponse = call read('classpath:shared/get-ciam-l2-token.feature') {username: '#(username)', "access_code":'#(access_code)', "device_id":'#(device_id)'}
	* def lvl2access_token = get lvl2TokenResponse.accessToken
	...
	* headers headers
	* header Authorization = 'Bearer ' + lvl2access_token
